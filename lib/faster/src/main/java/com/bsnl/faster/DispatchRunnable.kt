...
import com.bsnl.faster.state.TaskState
import com.bsnl.faster.utils.DispatcherLog
...
class DispatchRunnable : Runnable {
    ...
    override fun run() {
        ...
        DispatcherLog.i(
            mTask?.javaClass?.getSimpleName() + " begin run" + "  Situation  " + TaskState.currentSituation
        )
        ...
        if (!mTask!!.needCall() || !mTask!!.runOnMainThread()) {
            ...
            DispatcherLog.i(mTask?.javaClass?.getSimpleName() + " finish")
        }
    ...
    private fun printTaskLog(startTime: Long, waitTime: Long) {
        ...
        if (DispatcherLog.isDebug) {
            DispatcherLog.i(
                mTask?.javaClass?.getSimpleName() + "  wait " + waitTime + "    run "
                        ...
                        + "  Situation  " + TaskState.currentSituation
            )
        }
    }
}