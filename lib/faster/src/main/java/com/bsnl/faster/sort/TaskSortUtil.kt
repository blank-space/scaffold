...
object TaskSortUtil {
    private val sNewTasksHigh: MutableList<Task> =
        ArrayList() // 高优先级的Task
    ...
    fun getSortResult(
        ...
        for (i in originTasks.indices) {
            ...
            for (cls in task.dependsOn()!!) {
                val indexOfDepend =
                    getIndexOfTask(originTasks, clsLaunchTasks, cls)
        ...
        val newTasksAll =
            getResultTasks(originTasks, dependSet, indexList)
    ...
    private fun getIndexOfTask(
        ...
        for (i in 0 until size) {
            if (cls!!.simpleName == originTasks[i]!!::class.java.getSimpleName()) {
                return i
            }
        }
        return index
